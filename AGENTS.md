...
- Known deferred swap: `FormScreen` wraps RN's `KeyboardAvoidingView`; the community standard is **react-native-keyboard-controller** (`KeyboardAwareScrollView`). Adopt it when keyboard pain shows up (likely the real-auth forms) — the swap is contained inside `FormScreen`.
